...
// Fee schema: student (required), dueDate (required), feeType, month,
// originalAmount, netAmount, amount, description, notes
// receiptNo, paidAmount, status → all auto-generated by backend
...
export function FeeCollect() {
  ...
  useEffect(() => {
    studentsApi.list({ limit: 300 }).then(r => setStudents(r.data.data || [])).catch(() => {});
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.originalAmount || form.originalAmount <= 0) { toast.error('Amount must be greater than 0'); return; }
    if (!form.dueDate) { toast.error('Due date is required'); return; }

    setSaving(true);
    try {
      // Backend auto-generates: receiptNo, status, paidAmount
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" min={0} value={form.originalAmount} required
                onChange={e => set('originalAmount', Number(e.target.value))}
                className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Discount (%)</label>
              <input type="number" min={0} max={100} value={form.discountPercentage}
                onChange={e => set('discountPercentage', Number(e.target.value))}
                className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Due Date *</label>
              <input type="date" value={form.dueDate} required onChange={e => set('dueDate', e.target.value)}
                className="w-full px-3 py-2.5 rounded-lg border border-gray-300 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
            ...
            <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm">
              ...
              {form.discountPercentage > 0 && (
                <div className="flex justify-between mb-1 text-red-600">
                  <span>Discount ({form.discountPercentage}%)</span>
                  <span>− PKR {(form.originalAmount * form.discountPercentage / 100).toLocaleString()}</span>
                </div>
              )}
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => router.back()}
            className="px-5 py-2.5 rounded-xl border border-gray-300 text-sm font-medium hover:bg-gray-50 transition">Cancel</button>
...
export function OverdueFees() {
  ...
  useEffect(() => {
    feesApi.overdueList().then(r => setFees(r.data.data || [])).catch(() => toast.error('Failed to load')).finally(() => setLoading(false));
  }, []);
    ...
    <div className="space-y-5">
      <div className="page-header">
        <div>
          ...
          <p className="text-gray-500 text-sm">{fees.length} overdue records</p>
        </div>
      </div>
      ...
      {fees.length > 0 && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <p className="text-sm text-red-700"><strong>{fees.length} students</strong> have overdue payments. Contact them immediately.</p>
        </div>
      )}
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="data-table">
              <thead><tr><th>Student</th><th>Receipt</th><th>Fee Type</th><th>Balance</th><th>Due Date</th><th>Contact</th></tr></thead>
              <tbody>
                {fees.map((f: any) => (
